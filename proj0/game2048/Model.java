...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        for (int i = 0; i < board.size(); i++) {
            ...
            for (int k = values.length - 2; k >= 0; k--) {
                int tomove = -1;
                for (int k1 = top; k1 > k; k1--) {
                    if (values[k] != 0 && values[k1] == 0) {
                        tomove = Math.max(k1, tomove);
                    } else {
                        tomove = -1;
                    }
                }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        // TODO: Fill in this function.
        if (Model.emptySpaceExists(b) || Model.maxTileExists(b)) return true;
        for (Tile tile : b) {
            if (tile.col() < b.size() - 1 && tile.value() == b.tile(tile.col() + 1, tile.row()).value()) {
                return true;
            } else if (tile.row() < b.size() - 1 && tile.value() == b.tile(tile.col(), tile.row() + 1).value()) {
                return true;
            }
        }
        return false;
